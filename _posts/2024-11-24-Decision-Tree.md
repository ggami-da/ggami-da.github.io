...
각 노드는 그 노드를 선택한 데이터 집합을 가집니다. 이 때 노드에 속한 데이터의 클래스의 비율을 구하여 이를 그 노드의 조건부 확률 분포 $P(Y=k|X)_{\text{node}}$라고 정의한다.
...
정보획득량(information gain)는 $X$라는 조건에 의해 확률 변수 $Y$의 엔트로피가 얼마나 감소 하였는가를 나타내는 값입니다.  다음처럼 $Y$의 **`엔트로피`([참고](https://www.notion.so/b5b2ab10eea7449bb2193d97295d1e31?pvs=21))**에서 $X$에 대한 $Y$의 조건부 엔트로피를 뺀 값으로 정의됩니다.
...
{: .box-success}
지니 계수는 기본적으로 **불평등도**를 측정합니다.
- 노드 내 클래스가 얼마나 불균등하게 분포되어 있는지를 평가.
- 값이 0에 가까울수록 한 클래스만 포함된 순수한 노드.
- 값이 1에 가까울수록 여러 클래스가 고르게 섞인 불순한 노드.
